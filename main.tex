...
People with disabilities (again, that will be all of us \textit{eventually}) deserve to participate fully in life. They deserve financial independence. They also deserve loving care and interdependence~\cite{Bennett2018Interdependence,Mingus2011AccessIntimacy,Hsueh}. People with disabilities have a right to make informed decisions, to know about the status of a global pandemic, and to have an understanding of local and national politics~\cite{Fan2023Accessibility}. While we use visualizations to navigate all of these domains, the goal is not to make the charts and graphs themselves somehow equally useful to all people. That would be a false measurement of success.
...
Accessibility and data representation have been in conversation far longer than the modern visualization community has existed, and that conversation has two registers. The first is a history of techniques: a steady accumulation of ways to carry data to people through senses and devices other than a sighted eye on a screen. The second is a critical register, largely led by disabled scholars and disability studies, that questions how that technical work is framed, who leads it, and who it serves. This thesis tries to take both seriously, so we survey them in turn.
...
On the one hand, \textit{Chartability} was intended to be an in-depth set of heuristics that anyone could use to evaluate accessibility barriers in interactive data experiences. But on the other hand, the level of depth may also have introduced an unexpected outcome: many companies and organizations are interested in simply hiring an expert to help them remediate their systems. This has been great for hands-on collaboration, funding opportunities, and for generating the focus areas of this thesis. But it also points to a larger reality that many teams with the right motivations feel understaffed and under-resourced.
...
\begin{figure}[h]% specify a combination of t, b, p, or h for top, bottom, on its own page, or here
  \centering % avoid the use of \begin{center}...\end{center} and use \centering instead (more compact)
  \includegraphics[width=15cm,height=15cm,keepaspectratio, alt={A three-panel low-fidelity mockup of the Skeleton interface. Panel A shows the full graphical user interface, with a data visualization in the center surrounded by editing controls. Panel B shows nodes added as points placed over the marks of the chart. Panel C shows edges drawn as connecting lines between those nodes, forming navigation paths through the visualization.}]{skeleton}
  \caption{%
...
Long before accessibility, computing established a practice of taking structures that are not inherently visual and giving them a visual form that can be seen and acted upon directly. Sutherland's Sketchpad turned the constraints and topology of a drawing into a manipulable graphical object rather than coordinates in memory~\cite{Sutherland1963Sketchpad}. Iconic and visual programming environments such as Pygmalion rendered computation itself as arrangements of on-screen objects~\cite{Smith1977Pygmalion}, and syntax-directed editors like the Cornell Program Synthesizer treated a program not as text but as a hierarchical structure edited through its visible derivation tree~\cite{Teitelbaum1981Cornell}. Notations such as structured flowcharts gave a program's otherwise invisible control flow a fixed spatial form~\cite{Nassi1973Flowchart}. Myers later named the distinction this lineage had been drawing in practice for decades, separating \textit{visual programming}, writing programs through graphics, from \textit{program visualization}, depicting otherwise non-visual programs graphically~\cite{Myers1990Taxonomies}. A consequence of this history is easy to overlook: many structures we now treat as inherently visual, including trees, graphs, flowcharts, and the document object model, are abstractions whose visual rendering has become so dominant that the picture now stands in for the structure as we conceive of it. The tree is not the data; it is a representation we have agreed to think with, so thoroughly that its non-visual origin disappears from view.

This lineage matters for \textit{Skeleton} because it clarifies both what kind of artifact a navigation structure is and why giving it a visual form would change how practitioners work with it. Direct manipulation interfaces provide a continuous representation of the objects of interest, support actions on those objects, and give rapid, incremental feedback, properties that let people work with a structure they would otherwise hold in their heads or reach only through code~\cite{Shneiderman1983DirectManip, Hutchins1985DirectManip}. Grammars of interactive graphics extend the same move to authoring, letting designers write a visualization at the level of data fields rather than primitive marks~\cite{Satyanarayan2017VegaLite, Bostock2011D3}; the \textit{Dimensions API} applies that abstraction to traversal. Yet across this entire tradition, one structure has remained conspicuously without a visual form to manipulate: the navigation graph that a screen-reader user traverses. It is as abstract and as relational as a syntax tree or a flowchart, but it has never been given the visible, directly editable representation those structures acquired decades ago. \textit{Skeleton} asks what becomes possible when it is.
...
\textbf{Authoring the non-visual through a visual proxy.} A further pattern separates interactive navigation from these adjacent modalities: the others are not only experienced but \textit{authored} through visual surfaces. Sonification, whose output is sound, is designed in visual editors and declarative grammars that lay its structure out on screen~\cite{Kim2024}. Tactile graphics, whose output is touch, are composed by sighted transcribers on a visual canvas before being embossed~\cite{Bornschein2015Collaborative}. A tactile graphic can be touched and seen; a sonification can be heard and visualized. Interactive navigation structures are the exception: end users experience them through keyboard and screen reader, yet during authoring they have no visible form whatsoever. Where other non-visual modalities are designed by manipulating a visual proxy, navigation structure has been designed without one.
...
We built an \textit{Inspector} (\texttt{@data-navigator/inspector}) to render any Data Navigator structure as an interactive node-link graph using D3, with an accompanying console for debugging. The \textit{Inspector} accepts a structure object and draws its nodes and edges in one of two layouts: a force-directed graph for reading topology at a glance, or a tree layout for reading hierarchy and parent-child depth. Nodes are colored by a configurable field, defaulting to their level in the hierarchy, so that roots, divisions, and leaves are immediately distinguishable; directed edges are drawn as links, and the entry point is visually marked. 

Crucially, the \textit{Inspector} is passive: it does not drive navigation itself. A practitioner sets up Data Navigator's rendering and input modules on the actual chart as usual, and the \textit{Inspector} exposes \texttt{highlight()} and \texttt{clear()} methods that the practitioner wires into that navigation lifecycle. As the user traverses the real structure, the \textit{Inspector}'s focus indicator follows their position in the graph. This lets practitioners watch structure and reachability unfold during live navigation, verifying that the hierarchy has the right levels, that circular extents produce the expected wrap-around edges, and that a given path is reachable. An interactive console logs API information and underlying data when nodes are activated, and hovering or focusing a logged entry highlights the corresponding node in the graph. 
...
\noindent From a declaration like this, the build step does three things automatically. It assembles the nodes and the parent-child and sibling edges that connect them, including the cross-dimension edges that let a user move between the two hierarchies over shared leaves. It applies the boundary and childmost rules to decide what happens at each edge of the structure. And it assigns keyboard navigation rules to each dimension, drawing from a pool of directional key pairs; because that pool is finite, the API recommends keeping a structure to six dimensions or fewer and asks for explicit navigation rules beyond that point. The abstraction is chart-type-agnostic: bar charts, scatter plots, line charts, and layered charts all use the same vocabulary, with different combinations producing different navigation topologies. This directly addressed Bokeh's problem, where no native chart types exist to anchor a pattern: the API mirrors data fields and encoding choices as a set of dimensions, and a thin wrapper can map an arbitrary glyph assembly onto a handful of recurring dimensional shapes (\Cref{sec:opensource}).
...
The scaffold builds a Vega-Lite~\cite{Satyanarayan2017VegaLite} specification for the chosen chart type and renders it to a hidden, off-screen container, then extracts node positions from the rendered output and from the view's internal scale functions, using the rendering engine purely as a coordinate computation step: no actual chart is ever shown to the practitioner. Coincidentally, none of our co-designers were crafting visualizations using Vega or Vega-Lite (or derivatives), yet the rendering engine could faithfully reconstruct every necessary mark position over the underlying, inaccessible data visualization provided to \textit{Skeleton}.

Positions and outline strategies for category-level group nodes are then computed from the leaf positions using geometric algorithms: a union of child node paths, a convex hull, a bounding rectangle, or, for numerical dimensions, a grid or range box over the dimension's bounds. These designs were consistently produced as ideal treatments during co-design, so we chose them for our starting set of outline strategies.
...
\textbf{Specifying token patterns, editing instances.} Selecting any node populates a properties panel with spatial properties (position, size, shape) and semantic properties (ARIA role, description, and a label template editor; \Cref{fig:label}). The label template allows practitioners to assemble the text a screen reader will announce at that node from tokens drawn from data fields, including precise data values at leaf nodes and aggregate statistics at group-level nodes~\cite{Jones2024}. Value and field-name tokens reference any data field, while aggregate tokens compute count, sum, average, minimum, maximum, range, trend direction, and goodness of fit over a group's children, and every token in the template is resolved against real data so the preview shows what an actual node would announce rather than a placeholder. Editing labels can apply to all nodes of the same type or to a single instance.
...
P4's curiosity extended to alternative input modalities. Upon learning that \textit{Skeleton} supports text-based navigation and (due to leveraging Data Navigator) also supports a wide array of other input modalities, they asked, ``Can you talk at it, too? Is that what some people do?'' and followed up with, ``How cool is that? How do you make that good, though?'' P8 raised a concern about discoverability in the current interaction model: ``I've never used j or w to drill out, always shift arrow or option arrow\ldots'' and worried that a user might not know how to exit a nested level. These observations reflect a broad mental model of the user, from an abstract ``screen reader user'' to a person with multiple capabilities, preferences, and interaction patterns~(\ref{rq:R2s}).

\section{Discussion}

\subsection{Visibility as a Precondition for Iteration}

The central finding of this work is not that \textit{Skeleton} taught sighted practitioners how to design accessible navigation, but that it gave them something to react to. When navigation structure was invisible, our co-designers would only seek to verify whether navigation followed our design documentation. Once visible, questions shifted to whether it was good, why, how it could be improved, and what else it could do. Visibility encouraged iteration, and iteration enables continuous design improvements.
...
This reflective loop depends on having an external representation to perceive, and cognitive science offers an account of why a visual one can do work that a code listing cannot. A diagram is not merely an alternative encoding of the same information; by indexing what it shows spatially, it supports inferences that an informationally equivalent description leaves implicit~\cite{Larkin1987Diagram}. External representations are not simply inputs to thinking but part of the cognitive task itself, with reasoning distributed across the person and the artifact in front of them~\cite{Zhang1994Representations}, and graphical representations do this work precisely by offloading computation, constraining the inferences a viewer can draw, and re-representing a problem in a form that is easier to reason about~\cite{Scaife1996External}. We did not set out to test this literature, but it predicts what we observed: once the navigation structure was rendered as something practitioners could look at, the perceptual judgments they already apply to charts became available for a structure that had previously been legible only as code.
...
What visualization owes accessibility, then, is not simply better output but authoring tools that better stimulate reasoning, both individually and collaboratively, about design.

\subsection{Design Implications and Other Scenarios}

The mechanism behind \textit{Skeleton} is not specific to its current interface, and stating it as an implication makes it portable. The actionable claim for toolmakers is narrow: where a non-visual structure is authored without a visible representation, build one that the author can react to and manipulate, and the author will begin to design rather than merely comply. This is operational guidance, not a slogan. It identifies the missing artifact (a manipulable representation of the structure), the place it belongs (inside the authoring loop, not downstream of it as a validator), and the property that matters (the representation must talk back fast enough to support iteration). Tooling that surfaces structure only after a build, or only as static documentation, does not satisfy this even when it is technically accurate.

The scenarios in which this applies extend well past data visualization. Document reading order, web page landmark structure, and application focus order are all non-visual structures that sighted practitioners author today without visual feedback, verify slowly by hand with a screen reader, and therefore tend to ship as first drafts. \textit{Skeleton}'s techniques, an inspectable rendering of the structure, a declarative way to generate it from the content's own organization, a scaffold that places it over the existing layout, and a preview of what assistive technology will announce, transfer to each of these in principle. We do not claim to have demonstrated that transfer; we claim that the same gap exists in each domain and that the same class of intervention is tractable there. Within visualization, the most demanding open case remains bespoke graphics, where no template applies and the prior design question, whether any navigation structure is appropriate at all, has to be answered before \textit{Skeleton}'s authoring techniques become useful (see the subsection on bespoke visualizations as an unaddressed research problem).
...
While this chapter did not take the liberty of making a strong position statement, it is clear to me now that when viewed through the social model of disability, where disability is produced between human interaction with systems and environments, practitioners who are not traditionally considered ``people with disabilities'' in the medical sense may still face barriers to access when using an interface or system. \textit{Systems can disable any user.}

As another note worth making, we also attempted to ``vibe-code'' the prototype for this work as well, and encountered (ironically) many barriers while developing, both that we personally faced and that were unfortunately also produced for end users. Large-language models are not necessarily skilled out of the box, at the time of this writing, to make highly complex research prototypes that focus on cutting-edge accessibility functionality. The overall work required to vibe code now vastly outpaces whatever work we would have invested if we hadn't vibe-coded at all, because we will need to virtually rebuild this entire prototype from scratch before deploying it live for real-world use. It is, in this sense, a throwaway research prototype in its current condition. But at the same time, we did learn that vibe-coding in contexts with significant system complexity also takes from its user an opportunity to solve the engineering problems themselves, which is a challenge that we both enjoy and believe helps us to better understand our own work.
...
This project was also immensely difficult to recruit for. On the one hand, I had several blind data practitioners, researchers, and scientists who were colleagues, friends, and mentors supporting me. But few of them had the time or bandwidth to participate in our study. Recruiting locally was quite hard, so we decided to recruit non-data practitioners locally and, if we managed to get lucky, pull off running our studies ``in the location of our participant's choosing, should they consent to assume risk'' (as our IRB allowed). This specific verbiage meant that our participants could opt not to participate in our lab space on campus but rather choose their own location (with certain conditions). This ultimately enabled us to recruit blind data practitioners at conferences (ASSETS and CSUN, in particular), which made the study for this project stretch nearly a year and a half.

But it was worth it. Our participants were fantastic. And not to spoil the chapter's findings, but we discovered that hardware and input design might be the key to a whole new spectrum of studies where data interaction is framed as a fun and generative activity, and not just something dry, boring, or professionalized.
