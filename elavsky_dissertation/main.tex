...
\section{Preface}
\textit{Data Navigator} was a relatively successful research prototype: it evolved and grew and found a place in real-world work after its release. However, \textit{Data Navigator} eventually also became a tool to evoke new research directions. Notably, we made improvements to the system from a technical standpoint and also indentified new barriers that practitioners faced when applying \textit{Data Navigator}, both of which we discuss further in this chapter.

This work was proposed as the culmination of this thesis, and chronologically was the final work we completed for this body of work.
...
Perhaps in tension with the previous finding that tooling can reduce work, tooling may also \textit{increase} some kinds of work in exchange for quality or superior outcomes. The assumption for tooling, in a classic or naïve sense, is that tools exist to make work easier by reducing the labor and friction of certain tasks. But ease of work is only a goal if an existing baseline exists. If some work isn't being done or new design possibilities open up, then \textit{more}, not less, work might be the goal of an idealized tool. Friction can be useful, production, and may even be necessary. While outside of the scope of this thesis, we would urge fellow researchers to explore the tensions that arise in design when removing friction is the primary, if only, goal of a tool (as it appears to be the case with modern large language models and agentic tooling).

But it \textit{is} a reasonable assumption about practitioner tooling that its job is to make work smoother and easier. The evidence in this thesis complicates that assumption: in several places, the value of a tool came precisely from the resistance it introduced. \textit{Chartability} forced engagement with conflicts between guidelines (such as redundant encodings that help some users while overwhelming others) rather than smoothing those conflicts away; audits remained slow, and participants reported that the slowness helped them focus and prioritize. Evaluating a visualization for accessibility actually \textit{adds} time to the overall time it takes to make a visualization. \textit{Chartability} made it clear that if work isn't being done at all in the first place, that introducing new heuristics to a practitioner's workflow ultimately generates friction that wasn't there before. We view this as necessary and good.

\textit{Data Navigator} arguably introduced so much additional work and friction that it necessitated additional tools to make it easier to work with (in the \textit{Inspector}, \textit{Dimensions API}, and ultimately \textit{Skeleton} of Chapter 6). It became apparent that giving practitioners a nearly-infinite new space of design without clear scoping and bounds for that design space was burdensome and resulted in many additional barriers and design iterations. \textit{Data Navigator} could create \textit{any} discrete navigation experience from relationships in an information space. This is fantastic. But it appears that \textit{scope} in design is, in its own way, a source of friction \textit{and} a relief on the work required of that design: it limits and holds back what can be done, but this scoping friction in turn that reduces the labor of exploration required. Infinite, frictionless exploration of a design space appears to have put friction elsewhere in the work: during deliberation, handoffs, testing, and ultimately producing scope that serves to enforce through pre-chosen frictions, what the boundaries of a design should be.
