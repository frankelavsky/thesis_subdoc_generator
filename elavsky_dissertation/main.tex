...
This chapter sets up the story that the rest of this thesis engages. It makes three moves. First, I ground the idea that tools are not neutral instruments: tools shape the work and the workers who use them, which is why tool-making is a meaningful site for accessibility intervention. Second, I survey the long conversation between accessibility and data visualization and its history across modalities, including engaging critical voices that intersection with visualization and accessibility work which inform, support, and hold in tension the work of this thesis. Lastly, I argue that the hardest remaining challenges cluster in three domains, navigation, interaction, and personalization, and I signpost which chapters engage each of these.
...
The second register of the conversation is critical, and although it is sometimes treated as peripheral to systems research, it sets the terms under which systems research should be read. I treat it here as load-bearing. This section will be brief, but essential.
...
I unpack these three tooling gaps more below.
...
The package is developed in a public monorepo that has continued to grow since the original publication of this work. At the time of writing it also hosts companion packages built on the core library, including \texttt{@data-navigator/inspector}, a visual graph debugger for navigation structures, and the \textit{Skeleton} authoring application. I return to these in the following chapter, since this chapter concerns the core library.
...
This work was proposed as the culmination of my thesis, and chronologically was the final work I completed for this body of work.
...
\noindent\rule{16cm}{0.4pt}
\begin{quote}
This chapter was adapted from my paper, currently conditionally accepted with IEEE VIS:
\begin{adjustwidth}{1cm}{}
% \-\hspace{2cm}
...
As another note worth making, I also attempted to ``vibe-code'' the prototype for this work as well, and encountered (ironically) many barriers while developing, both that I faced and that were produced for end users. Large-language models are not necessarily skilled out of the box, at the time of this writing, to make highly complex research prototypes that focus on cutting-edge accessibility functionality. The overall work required to vibe code now vastly outpaces whatever work I would have invested if I hadn't vibe-coded at all, because I will need to virtually rebuild this entire prototype from scratch before deploying it live for real-world use. It is, in this sense, my first throwaway research prototype. But at the same time, I did learn that vibe-coding in contexts with significant system complexity also takes from me an opportunity to solve the engineering problems myself, which is a challenge that I both enjoy and believe help me to better understand my own work.

Perhaps in the future, I will submit an experience report or write a workshop paper on the \textit{self} and \textit{social} barriers that vibe-coding introduced into my accessibility-centered prototyping work.
...
\noindent\rule{16cm}{0.4pt}
\begin{quote}
This chapter was adapted from my paper, soon to be submitted to TVCG:
\begin{adjustwidth}{1cm}{}
% \-\hspace{2cm}
...
Perhaps in tension with the previous finding that tooling can reduce work, tooling may also \textit{increase} some kinds of work in exchange for quality or superior outcomes. The assumption for tooling, in a classic or naïve sense, is that tools exist to make work easier by reducing the labor and friction of certain tasks. But ease of work is only a goal if an existing baseline exists. If some work isn't being done or new design possibilities open up, then \textit{more}, not less, work might be the goal of an idealized tool. Friction can be useful, production, and may even be necessary. While outside of the scope of this thesis, I would urge fellow researchers to explore the tensions that arise in design when removing friction is the primary, if only, goal of a tool (as it appears to be the case with modern large language models and agentic tooling).

But it \textit{is} a reasonable assumption about practitioner tooling that its job is to make work smoother and easier. The evidence in this thesis complicates that assumption: in several places, the value of a tool came precisely from the resistance it introduced. \textit{Chartability} forced engagement with conflicts between guidelines (such as redundant encodings that help some users while overwhelming others) rather than smoothing those conflicts away; audits remained slow, and participants reported that the slowness helped them focus and prioritize. Evaluating a visualization for accessibility actually \textit{adds} time to the overall time it takes to make a visualization. \textit{Chartability} made it clear that if work isn't being done at all in the first place, that introducing new heuristics to a practitioner's workflow ultimately generates friction that wasn't there before. I view this as necessary and good.
...
I would argue that the \textit{cross-feelter} device from \textit{Cross-perception} was a ``classic'' tool in this sense, unlike the rest: it reduced friction \textit{and} reduced work, which led to more productivity. It is good to hold this project in tension with all of the others in this thesis, which introduced new, productive frictions into accessibility work in some way or another and may or may not have made work easier in some ways in the process. \textit{Cross-perception}, by comparison, primarily served to open up a space of design while we demonstrated one design within that space through our hardware device.
